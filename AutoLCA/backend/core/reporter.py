...
class JRCReporter:
    ...
    def generate_pdf_buffer(self, process_data):
        ...
        # 1.1 Executive AI Summary (CSO Verdict)
        ai_verdict = meta.get("ai_verdict")
        if ai_verdict:
            elements.append(Paragraph("1.1 EXECUTIVE AI SUMMARY", subhead_style))
            # Styled container for AI text
            ai_style = ParagraphStyle(
                ...
                backColor=colors.slate[50], 
                borderPadding=10,
                borderRadius=8
            )
            
            # Split by double newline to maintain paragraph structure in PDF
            paragraphs = ai_verdict.split("\n\n")
            for p in paragraphs:
                if p.strip():
                    elements.append(Paragraph(p.strip(), ai_style))
                    elements.append(Spacer(1, 10))
            
            elements.append(Paragraph("<i>Note: This section is generated by Triya Strategic AI utilizing the Gemini-1.5 model.</i>", 
                                      ParagraphStyle('AiNote', parent=styles['Normal'], fontSize=7, textColor=colors.gray)))
            elements.append(Spacer(1, 15))
        ...
        if snapshot_b64 and isinstance(snapshot_b64, str) and "," in snapshot_b64:
            try:
                ...
                img.hAlign = 'CENTER'
                elements.append(Paragraph("2.0 INVENTORY ANALYSIS (SUPPLY CHAIN GRAPH)", subhead_style))
                elements.append(img)
            except Exception as e:
                logger.error(f"Graph snapshot decode failed: {e}")

        # 3.1 Impact Visualization (Recharts Snapshot)
        chart_snapshot = meta.get("chart_snapshot")
        if chart_snapshot and isinstance(chart_snapshot, str) and "," in chart_snapshot:
            try:
                chart_data = base64.b64decode(chart_snapshot.split(",")[1])
                chart_buffer = io.BytesIO(chart_data)
                chart_img = Image(chart_buffer, width=450, height=250)
                chart_img.hAlign = 'CENTER'
                elements.append(Paragraph("3.1 IMPACT VISUALIZATION DASHBOARD", subhead_style))
                elements.append(chart_img)
                elements.append(Spacer(1, 10))
            except Exception as e:
                logger.error(f"Chart snapshot decode failed: {e}")
